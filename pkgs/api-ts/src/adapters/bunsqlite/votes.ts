...
export class BunSQLiteVotesAdapter implements VotesDataAdapter {
  ...
  public async voteUpThread(accountId: string, threadId: string): Promise<Msg<Vote>> {
    return this.createOrUpdateVote(accountId, threadId, null, "up");
  }

  public async voteDownThread(accountId: string, threadId: string): Promise<Msg<Vote>> {
