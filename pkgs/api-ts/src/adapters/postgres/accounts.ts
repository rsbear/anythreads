...
export class PostgresAccountsAdapter implements AccountsDataAdapter {
  ...
  public async update(id: string, account: AccountUpdate): Promise<Msg<Account>> {
    try {
  ...
  public async ban(accountId: string, bannedUntil: Date | null): Promise<Msg<Account>> {
    try {
  ...
  public async findMany(options?: AccountsFindManyOptions): Promise<Msg<Account[]>> {
    try {
  ...
  public async votesInThread(opts: VotesInThreadInput): Promise<Msg<VotesInThread>> {
    ...
    try {
      ...
      for (const vote of result.rows) {
        const key = vote.reply_id ? `reply:${vote.reply_id}` : `thread:${vote.thread_id}`;
        voteHash[key] = {
