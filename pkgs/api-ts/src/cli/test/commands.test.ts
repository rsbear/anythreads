...
describe("CLI Commands", () => {
	...
	test("should throw error for unknown command", async () => {
		...
		expect(executor.execute("unknown", config, [])).rejects.toThrow(
			"Unknown command",
		);
	});
	...
	test("setup command should require target argument", async () => {
		...
		expect(executor.execute("setup", config, [])).rejects.toThrow(
			"Usage: setup",
		);
	});
	...
	test("empty command should require target argument", async () => {
		...
		expect(executor.execute("empty", config, [])).rejects.toThrow(
			"Usage: empty",
		);
	});
	...
	test("setup command should accept valid table names", async () => {
		...
		expect(
			executor.execute("setup", config, ["accounts"]),
		).resolves.toBeUndefined();
		expect(
			executor.execute("setup", config, ["threads"]),
		).resolves.toBeUndefined();
		expect(
			executor.execute("setup", config, ["replies"]),
		).resolves.toBeUndefined();
