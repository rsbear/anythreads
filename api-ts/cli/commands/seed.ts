  ...
  for (const reply of seedData.replies) {
    ...
    if (config.dbType === "bun_sqlite") {
      ...
      await config.db.query(
        ...
        [reply.id, reply.threadId, reply.accountId, reply.body, reply.replyToId, now, now, {}],
      );
    }
  }
...
export const seedCommand: Command = {
  ...
  handler: async (config) => {
    ...
    console.log("Database seeded successfully with 3 accounts, 2 threads, 8 replies, and 10 votes");
  },
};
